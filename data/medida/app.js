angular.module('scotchApp').controller('medidaController', function ($scope, $route) {
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector).jqGrid({
	        url: 'data/medida/xml_unidades_medida.php',
	        autoencode: false,
	        datatype: "xml",
			height: 330,
			colNames:['ID','UNIDAD','ABREBIATURA','CANTIDAD','PRINCIPAL','OBSERVACIONES','FECHA CREACIÓN'],
			colModel:[
				{name:'id',index:'id', frozen:true,align:'left',search:false,editable: true, hidden: true, editoptions: {readonly: 'readonly'}},
				{name:'nombre_unidad',index:'nombre_unidad',width:150, editable:true, editoptions:{size:"20", maxlength:"30"}, editrules: {required: true}},
				{name:'abreviatura',index:'abreviatura',width:150, editable:true, search:false, editoptions:{size:"20", maxlength:"30"}, editrules: {required: true}},
				{name:'cantidad',index:'cantidad',width:150, editable:true, search:false, editrules: {required: true}, editoptions:{maxlength: 30, dataInit: function(elem){$(elem).bind("keypress", function(e) {return ValidNum(e)})}}},
	    ...
	    },
	    {
	        ...
	        afterSubmit: function(response)  {
                ...
                if(retorno == '2'){
                	...
                	if(retorno == '3') {
                		$("#nombre_unidad").val("");
	                	return [false,"Error.. La Unidad de Medida ya fue Agregada"];
	                }
                }
                return [true,'',retorno];
            },
