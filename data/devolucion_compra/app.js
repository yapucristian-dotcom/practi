angular.module('scotchApp').controller('devolucion_compraController', function ($scope, $interval) {
	...
	jQuery(function($) {
	    ...
	    function combo1(tipo) {
	        $.ajax({
	            type: "POST",
	            url: "data/devolucion_compra/app.php",
	    ...
	    function barras() {
            ...
            $.getJSON('data/devolucion_compra/search.php?codigo_barras=' + codigo_barras, function(data) {
            	if(data == null) {
					swal({
		...
		// abrir en una nueva ventana reporte retenciones
		...
		$('#btn_0').click(function() {
			var formulario = $("#form_devolucion").serialize();
		...
		// reimprimir devolucion
		$('#btn_3').click(function() {
			if($('#id_devolucion').val() == '') {
				$.gritter.add({
					title: 'Seleccione Devolución a Reimprimir',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});	
			} else {
				var id = $('#id_devolucion').val();
				var myWindow = window.open('data/reportes/devolucion_compra.php?hoja=A5&id='+id,'popup','width=900,height=650');
			}
		});
		// fin
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	    	datatype: "xml",
		    url: 'data/devolucion_compra/xml_factura_compra.php',         
	        autoencode: false,
