angular.module('scotchApp').controller('ingresosController', function ($scope, $interval) {
	...
	jQuery(function($) {
	    ...
	    function barras() {
            ...
            $.getJSON('data/ingresos/search.php?codigo_barras=' + codigo_barras, function(data) {
            	if(data == null) {
					swal({
		...
		$scope.methodspdf = function(id) { 
			var myWindow = window.open('data/reportes/ingreso.php?hoja=A5&id='+id,'popup','width=900,height=650');
		} 
		// fin
		...
		$('#btn_0').click(function() {
			var formulario = $("#form_ingreso").serialize();
			var submit = "btn_guardar";
			var filas = jQuery("#table").jqGrid("getRowData");

			if(filas.length == 0) {
                $.gritter.add({
					title: 'Ingrese productos a los Ingresos',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});
                $('#codigo').focus();	
                ...
                $.ajax({
                    ...
                    success: function(data) {
		        		...
		        		bootbox.alert("Gracias! Por su Información Ingreso Agregado Correctamente!", function() {
						  	var myWindow = window.open('data/reportes/ingreso.php?hoja=A5&id='+id,'popup','width=900,height=650'); 
						  	location.reload();
						});
                    }
		...
		$('#btn_3').click(function() {
			if($('#id_ingreso').val() == '') {
				$.gritter.add({
					title: 'Seleccione Ingreso a Reimprimir',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});	
			} else {
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	    	datatype: "xml",
		    url: 'data/ingresos/xml_ingresos.php',         
	        autoencode: false,
	        colNames: ['ID','ORIGEN','DESTINO','FECHA EMISIÓN','TOTAL','ACCIÓN'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	            {name:'I.origen',index:'I.origen', frozen:true, align:'left', search:true, hidden: false},
	            {name:'I.destino',index:'I.destino',frozen : true,align:'left', search:true, width: '300px'},
	        ...
	        gridComplete: function() {
				...
				for(var i = 0;i < ids.length;i++) {
					var id_ingreso = ids[i];
					pdf = "<a onclick=\"angular.element(this).scope().methodspdf('"+id_ingreso+"')\" title='Reporte Ingreso' ><i class='fa fa-file-pdf-o red2' style='cursor:pointer; cursor: hand'> PDF</i></a>"; 
	        ...
	        ondblClickRow: function(rowid) {     	            	            
            	...
            	$.ajax({
					url: 'data/ingresos/app.php',
					type: 'post',
					data: {llenar_cabezera_ingreso:'llenar_cabezera_ingreso',id: ret.id},
					dataType: 'json',
					success: function (data) {
				...
				$.ajax({
					url: 'data/ingresos/app.php',
					type: 'post',
					data: {llenar_detalle_ingreso:'llenar_detalle_ingreso',id: ret.id},
					dataType: 'json',
					success: function (data) {
