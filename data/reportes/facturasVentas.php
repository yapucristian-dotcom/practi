<?php
    ...
    $pdf->Cell(15, 6, utf8_decode('Iva 18%'),1,0, 'C',0);                                     
    ...
    while ($row = $class->fetch_array($resultado)) {        
        $resultado1 = $class->consulta("SELECT F.serie, F.fecha_actual, F.hora_actual, F.fecha_cancelacion, F.tipo_precio, F.forma_pago, F.tarifa0, F.tarifa, F.iva_venta, F.descuento_venta, F.total_venta, C.identificacion, C.nombres_completos, F.id FROM factura_venta F, clientes C WHERE F.id_cliente = C.id AND F.id_cliente = '$row[0]' and fecha_actual between '$_GET[inicio]' and '$_GET[fin]' order by F.id asc");                        
