angular.module('scotchApp').controller('kardexController', function ($scope, $interval) {
	...
	jQuery(function($) {
	    ...
	    function barras() {
            ...
            $.getJSON('data/kardex/search.php?codigo_barras=' + codigo_barras, function(data) {
            	if(data == null) {
					swal({
		...
		$('#btn_consultar').click(function() {
			var formulario = $("#form_kardex").serialize();
			$('#dynamic-table').dataTable().fnClearTable();

			$.ajax({
				url: 'data/kardex/app.php',
				type: 'post',
				data: formulario + "&cargar_tabla=" + 'cargar_tabla',
				dataType: 'json',
				success: function(response) {
					if (response == null) {
						swal({
			                title: "Lo sentimos Kardex no Encontrado",
			                type: "warning",
			            }); 	
					} else {
						...
						for (var i = 0; i < response.length; i++) {
							tabla.row.add([
						            ...
						            response[i]['fecha_kardex'],
						            response[i]['detalle'],
						            response[i]['cantidad'],
						            response[i]['valor_unitario'],
						            response[i]['total'],
						            response[i]['saldo']
			                ]).draw(false);                                 
				        }	
